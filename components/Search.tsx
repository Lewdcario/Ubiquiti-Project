...
const Search: React.FC<SearchProps> = ({ searchTerm, setSearchTerm, suggestions, ...rest }) => {
	...
	const renderSuggestion = (suggestion: Suggestion, query: string) => {
		const matchIndex = suggestion.deviceName.toLowerCase().indexOf(query.toLowerCase());
		const prefix = suggestion.deviceName.slice(0, matchIndex);
		const match = suggestion.deviceName.slice(matchIndex, matchIndex + query.length);
